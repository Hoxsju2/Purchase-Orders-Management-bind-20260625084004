...
class WCSOM_Frontend {
    ...
    public function handle_supplier_update() {
        // Check for required inputs and nonce security
        if (!isset($_POST['po_id']) || !isset($_POST['wcsom_nonce'])) {
            wp_die('Missing required form parameters. Please go back and try again.');
        }

        if (!wp_verify_nonce($_POST['wcsom_nonce'], 'wcsom_update_po')) {
            wp_die('Security check failed. Please refresh the page and try submitting again.');
        }
        ...
        if (in_array($status, ['draft', 'waiting_for_quote']) && isset($_FILES['wcsom_attachments']) && isset($_FILES['wcsom_attachments']['name']) && !empty($_FILES['wcsom_attachments']['name'][0])) {
            ...
            // Backup the original $_FILES array
            ...
            for ($i = 0; $i < $file_count; $i++) {
                if (!empty($files['name'][$i])) {
                    ...
                    // Temporarily mock the global $_FILES array for WordPress media handle upload
