...
<head>
    <meta charset="UTF-8">
    <title>Admin Purchase Order - <?php echo esc_html($order->post_title); ?></title>
    ...
    <style>
        ...
        .po-title h1 { margin: 0; color: #0f172a; font-size: 28px; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 800; }
        .po-title p { margin: 6px 0 0 0; font-size: 14px; color: #475569; }
        .admin-badge { display:inline-block; background: #1e293b; color: #fff; padding: 4px 10px; border-radius: 4px; font-size: 12px; font-weight: 700; margin-bottom: 10px; letter-spacing: 1px;}
        ...
        .table th { background: #f1f5f9; padding: 12px 10px; text-align: left; font-size: 11px; text-transform: uppercase; color: #475569; border-bottom: 2px solid #e2e8f0; font-weight: 700; letter-spacing: 0.025em; }
        .table td { padding: 12px 10px; border-bottom: 1px solid #e2e8f0; vertical-align: middle; color: #0f172a; font-size: 13px; }
        
        .thumb-wrapper { width: 40px; height: 40px; border-radius: 6px; overflow: hidden; border: 1px solid #e2e8f0; display: inline-block; background-color: #f8fafc; }
...
<body>
    ...
    <div class="po-container" id="po-content">
        <div class="header">
            ...
            <div class="po-title">
                <span class="admin-badge">INTERNAL ADMIN PO</span>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                foreach ($items as $item): 
                    ...
                    // Difference logic
                    $diff_html = '<span style="color:#64748b;">0%</span>';
                    if ($orig_price > 0) {
                        $diff = (($price - $orig_price) / $orig_price) * 100;
                        if ($diff > 0) {
                            $diff_html = '<span style="color:#dc2626; font-weight:bold;">+' . number_format($diff, 1) . '%</span>';
                        } elseif ($diff < 0) {
                            $diff_html = '<span style="color:#16a34a; font-weight:bold;">' . number_format($diff, 1) . '%</span>';
                        }
                    } else {
                        $diff_html = '<span style="color:#94a3b8; font-size:11px;">N/A</span>';
                    }
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <strong style="font-size: 14px; <?php echo $is_added_by_supplier ? 'color: #dc2626;' : ''; ?>"><?php echo esc_html($product->get_name()); ?></strong>
                        <?php if ($is_published): ?></a><?php endif; ?>
                    ...
                    <td style="text-align: right;"><strong style="font-size: 14px;"><?php echo wcsom_format_usd($line_total); ?></strong></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <script>
        function downloadPDF() {
            ...
            var opt = {
                ...
                filename:     'admin-<?php echo esc_js($order->post_title); ?>.pdf',
